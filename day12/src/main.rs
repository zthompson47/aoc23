...
fn part2() -> Vec<u32> {
    input().iter().fold(Vec::new(), |mut result, row| {
        ...
        for solution in Solutions::new(row.string.clone(), &row.groups, possible_slots) {
...
impl Solutions {
    fn new(string: String, groups: &[u32], possible_slots: Vec<Vec<usize>>) -> Self {
        ...
        let index = Some(vec![0; groups.len()]);
        Solutions {
            possible_slots,
...
impl Iterator for Solutions {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(index) = &mut self.index {
            ...
            //'out: while !found_valid_solution {

            // Update index of permutation
            /*for (i, group) in index.iter_mut().enumerate() {
                *group += 1;
                if group >= &mut self.possible_slots[i].len() {
                    ...
                    *group = 0;
                    ...
                    let start_idx = self.possible_slots[i][*group];
            ...
            for i in 0..index.len() {
                ...
                if index[i] >= self.possible_slots[i].len() {
                    ...
                    let group_len = self.groups[i] as usize;
                    let next_idx = self.possible_slots[i + 1][index[i + 1]];
                    //println!("start_idx {start_idx}, group_len {group_len}, next_idx {next_idx}");
                    if start_idx + group_len >= next_idx {
                        //println!("  SKIP");
                        index[i] = 0;
                    } else {
            ...
            //    let group_len = self.groups[i] as usize;
            //    let next_idx = self.possible_slots[i + 1][index[i + 1]];
            //    println!("--> {i} {} {} {}", start_idx, group_len, next_idx);
            //    if start_idx + group_len >= next_idx {
